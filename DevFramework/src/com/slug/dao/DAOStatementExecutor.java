...
public class DAOStatementExecutor extends DAOSqlHandler {
	...
	public DAOStatementExecutor(){
  	...
  	/**
   	 ...
   	 * @return
   	 */
	public ResultSet doExecuteSelectLoggable(Connection conn, PreparedStatement pstmt,ResultSet rs,Box ubox, String input_value[] ){
		...
		try {
		   ...
		   Logging.err.println(this.getClass().getName()+"."+"doExecuteSelect()"+"=>\n" + se+"\n" + query);
			for ( int input = 0 ; input < input_value.length ; input++ )
			{
	 ...
	 * @throws PException
	 */
    public String[] updateObject(Connection pconn, Box ubox , String input_value[]) throws PException
	{
		...
		try
		{
			if(pconn == null){
				...
				pstmt = new LoggableStatement(pconn,query);//CLAB 처리를 위한 LaggableStatement 사용
			}
